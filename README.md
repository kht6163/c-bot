...
- **프로젝트**: 왼쪽 트리에서 폴더를 고른다. 코딩 세션은 그 프로젝트 아래에 매달린다. 폴더를 눌러도 열린 대화는 그대로다
...
2. 왼쪽 **프로젝트** 트리에서 폴더를 고릅니다. 섹션 `+`로 다른 폴더를 열 수 있고, 대화 상자에 **실행한 폴더 열기**가 있으면 `bun run dev`를 켠 저장소가 바로 열립니다. 프로젝트 행의 `+`는 그 폴더에 새 세션을 만듭니다.
