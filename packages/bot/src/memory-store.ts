...
export class MemoryStore {
  ...
  create(input: { title: string; body: string }): MemoryEntry {
    ...
    this.db
      .query(
        `INSERT INTO memories (id, title, body, search, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)`,
      )
      .run(id, storedTitle, body, cjkSearchText(`${storedTitle}\n${body}`), now, now);
    return this.get(id) as MemoryEntry;
  }

  update(id: string, patch: { title?: string; body?: string }): MemoryEntry | undefined {
    ...
    this.db
      .query(
        `UPDATE memories SET title = ?, body = ?, search = ?, updated_at = ? WHERE id = ?`,
      )
      .run(storedTitle, body, cjkSearchText(`${storedTitle}\n${body}`), now, id);
    return this.get(id);
  }
  ...
  search(query: string, limit = MEMORY_SEARCH_LIMIT): MemoryHit[] {
    ...
    try {
      const rows = this.db
        .query(
          `SELECT m.id, m.title, m.body, m.created_at AS createdAt, m.updated_at AS updatedAt,
